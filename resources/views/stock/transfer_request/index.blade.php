...
@section('titles')
    <title>Keyonline - Item Transfer Request</title>
@endsection
                        ...
                        <form autocomplete="off" id="filter-form">
                            <div class="d-flex flex-row align-items-end">
                                <div>
                                    ...
                                    <input type="text" class="js-flatpickr form-control" id="date" name="date" value="{{ request()->get('date' ?? '') }}">
                                </div>
                                <button type="submit" class="btn btn-secondary ms-3" id="filter"><i class="fa fa-fw fa-filter"></i>Filter</button>
                            </div>
                        </form>
                    </div>
                    @if(auth()->user()->hasAnyPermission(['admin', 'transfer_request.add']))
                        <div class="col-auto">
                            <a class="btn btn-primary" href="{{ route('transfer_request.add') }}"><i class="fa fa-fw fa-plus"></i> Add</a>
                        </div>
                    @endif
                </div>
                        ...
                        <div class="table-responsive w-100">
                            <table class="table table-bordered table-striped table-vcenter table-fixed" id="datatable">
                                ...
                                <tr>
                                    ...
                                    <th class="text-center">Need For</th>
...
@section('scripts')
    ...
    <script>
        $(function() {
            @if(session('type') == 'success' && str_contains(session('message'), 'Item Transfer Request successfully added'))
                FormPreserver.clear('transfer_request_form_data');
            @endif
            ...
            $('#datatable').DataTable({
                ...
                responsive: false,
                ajax: {
                    url: '{!! route('transfer_request.datatable') !!}',
                    data: {
                ...
                columns: [
                    ...
                    { data: 'WarehouseFromName', name: 'WarehouseFromName', class: 'text-center'},
                    { data: 'WarehouseToName', name: 'WarehouseToName', class: 'text-center'},
                    { data: 'NeedFor', name: 'NeedFor', class: 'text-center' },
            ...
            $('#datatable').on('click', '.delete-btn', function () {
                ...
                Swal.fire({
                    icon: 'info',
                    title: 'Delete Item Transfer Request',
                    text: "Are you sure you want to delete this item transfer request?",
                    showDenyButton: true,
