...
@section('titles')
    <title>Keyonline - Edit Item Transfer Request</title>
@endsection
...
@section('content')
    ...
    <div class="px-lg-5 py-lg-3 p-3" id="vue-container">
        <form autocomplete="off" method="post" enctype="multipart/form-data" action="{{ route('transfer_request.update') }}" id="transfer-form">
            @csrf
                        ...
                        <div class="col-lg-8 col-12">
                            <div class="row align-items-center mb-3">
                                <div class="col-lg-3 col-12 mb-lg-0 mb-3">
                                    ...
                                    <input type="text" class="form-control" value="{{ $request->TransactionNo }}" disabled>
                                </div>
                                <div class="col-lg-3 col-12">
                                    <label class="form-label">Transaction Date <span class="text-danger">*</span></label>
                                    <input type="text" class="js-flatpickr form-control" id="TransactionDate" name="TransactionDate" placeholder="d/m/Y" value="{{ date('d/m/Y', strtotime($request->TransactionDate)) }}" readonly="readonly" required>
                                </div>
                                <div class="col-lg-3 col-12">
                                    <label class="form-label">Expire Date</label>
                                    <input type="text" class="js-flatpickr form-control" id="ExpiredDate" name="ExpiredDate" placeholder="d/m/Y" value="{{ $request->ExpiredDate ? date('d/m/Y', strtotime($request->ExpiredDate)) : '' }}" readonly="readonly">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="block-content">
                    <div class="row justify-content-between mb-3">
                        <div class="col-lg-4 col-12 mb-lg-0 mb-3">
                            ...
                            <textarea name="NeedFor" class="form-control" rows="3">{{ old('NeedFor') ?? $request->NeedFor }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        let app = new Vue({
            ...
            data: {
                isLoading: false,
                initiate: false,
                fromId: '{{ old('WarehouseIDFrom') ?? $request->WarehouseIDFrom }}',
                staffFrom: '{{ old('StaffInChargeIDFrom') ?? $request->StaffInChargeIDFrom }}',
                toId: '{{ old('WarehouseIDTo') ?? $request->WarehouseIDTo }}',
                staffTo: '{{ old('StaffInChargeIDTo') ?? $request->StaffInChargeIDTo }}',
                details: {!! json_encode($request->details->map(function($d) {
                    return [
                        'id' => $d->id,
                        'part' => $d->PartID,
                        'qty' => (float)$d->Qty,
                        'unit' => $d->UnitID,
                        'unitUrl' => route('misc.partunit2', ['id' => $d->PartID]),
                        'conversion' => 1,
                        'stock' => 0,
                        'initiated' => false,
                    ];
            ...
            mounted() {
                ...
                $('#transfer-form').on('change', '#WarehouseIDFrom', function (){
                    let val = $(this).val();
                    app.fromId = val;
                    if(!val){
                        app.staffFrom = null;
                ...
                $('#transfer-form').on('change', '#StaffInChargeIDFrom', function (){
                    app.staffFrom = $(this).val();
                });

                $('#transfer-form').on('change', '#WarehouseIDTo', function (){
                    let val = $(this).val();
                    app.toId = val;
                    if(!val){
                        app.staffTo = null;
                ...
                $('#transfer-form').on('change', '#StaffInChargeIDTo', function (){
                    app.staffTo = $(this).val();
                });

                $('#transfer-form').on('change', '.part', function (){
                    let index = $(this).data('index');
                    if($(this).val() != null){
                        app.details[index].part = $(this).val(); // Sync model
                        app.changePart(index);
                    }
                });

                $('#transfer-form').on('change', '.unit', function (){
                    let index = $(this).data('index');
                    if($(this).val() != null){
                        app.details[index].unit = $(this).val(); // Sync model
                ...
                // Initial load for conversion and stock
                this.$nextTick(() => {
                    setTimeout(() => {
                        for(var i = 0; i < this.details.length; i++){
                            this.getConversion(i);
                            this.getStock(i);
                        }
                    }, 500);
                });
