...
@section('titles')
    <title>Keysoft NLA WMS - Add Advanced Stock Adjustment</title>
@endsection

@section('content')
    <div class="px-lg-5 py-lg-3 p-3" id="vue-container">
        <form autocomplete="off" method="post" action="{{ route('adjust.store') }}" id="adjust-form">
            @csrf
            ...
            <div class="d-flex flex-row align-items-center mb-5">
                ...
                <h1 class="h3 fw-bold ms-4 mb-0">Add Stock Adjustment</h1>
            </div>
                ...
                <div class="block-content pb-3">
                    ...
                    <div class="row align-items-center mb-3">
                        <div class="col-lg-2 col-12 pe-lg-0">
                            ...
                            <input type="text" name="TransactionNo" id="TransactionNo" class="form-control" v-model="transactionNo" :required="!automatic" :disabled="automatic">
                        </div>
                        ...
                        <div class="col-lg-6 col-12 pe-lg-5">
                            <div class="row mb-3">
                                <div class="col-lg-6 col-12">
                                    ...
                                    <input type="text" class="js-flatpickr form-control" id="TransactionDate" name="TransactionDate" placeholder="d/m/Y" v-model="transactionDate" readonly required>
                                </div>
                                <div class="col-lg-6 col-12">
                                    <label class="form-label">Expired Date <span class="text-danger">*</span></label>
                                    <input type="text" class="js-flatpickr form-control" id="ExpiredDate" name="ExpiredDate" placeholder="d/m/Y" v-model="expiredDate" readonly required>
                                </div>
                            </div>

                            <div class="mb-3">
                                <label class="form-label">Division <span class="text-danger">*</span></label>
                                <select2 url="{{ route('misc.division2') }}" v-model="division" :prevalue="division" class="form-select" name="DivisionID" id="DivisionID" required>
                                    <option value="">-</option>
                                </select2>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="space-y-2">
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" id="tOpname" name="Type" value="opname" v-model="type">
                                        <label class="form-check-label" for="tOpname">Stock Opname</label>
                                    </div>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" id="tManual" name="Type" value="manual" v-model="type">
                        ...
                        <div class="col-lg-6 col-12 pe-lg-5">
                            <div v-if="type === 'manual'">
                                <div class="mb-3">
                                    ...
                                    <select2 url="{{ route('misc.warehouse2') }}" v-model="warehouse" :prevalue="warehouse" class="form-select" name="WarehouseID" id="WarehouseID" required>
                                        <option value="">-</option>
                                    </select2>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">Inventory Type</label>
                                    <select2 url="{{ route('misc.type', ['select2' => true, 'all' => true]) }}" v-model="invType" :prevalue="invType" class="form-select" name="InventoryTypeID" id="InventoryTypeID">
                            ...
                            <div v-if="type === 'opname'">
                                <div class="mb-3">
                                    ...
                                    <select2 url="{{ route('adjust.opname') }}" v-model="opname" :prevalue="opname" class="form-select" name="StockOpnameNo" id="StockOpnameNo" required>
                                        <option value="">-</option>
                                    </select2>
                                </div>
                                <div class="mb-3">
                    ...
                    <div class="table-responsive w-100">
                        <table class="table table-bordered nowrap w-100" style="table-layout: fixed">
                            ...
                            <tr v-for="(detail, index) in checkers" :key="detail.id">
                                ...
                                <td>
                                    <select class="form-select" name="status[]" v-model="detail.status">
...
@section('scripts')
    ...
    <script>
        ...
        const initialCheckers = @json($checkers ?: []);
        let app = new Vue({
            el: '#vue-container',
            data: {
                isRestoring: false,
                transactionNo: @json(old('TransactionNo') ?? ''),
                automatic: @json(old('automatic') ? true : (count($errors->all()) > 0 ? false : true)),
                transactionDate: @json(old('TransactionDate') ?? date('d/m/Y')),
                expiredDate: @json(old('ExpiredDate') ?? date('d/m/Y')),
                notes: @json(old('Notes') ?? ''),
                type: '{{ old('Type') ?? 'opname' }}',
                opname: '{{ old('StockOpnameNo') ?? '' }}',
                warehouse: '{{ old('WarehouseID') ?? '' }}',
                division: '{{ old('DivisionID') ?? '' }}',
                invType: '{{ old('InventoryTypeID') ?? '' }}',
            ...
            methods: {
                ...
                saveModalDetail() {
                    if (!this.modal.PartID) {
                        return Swal.fire('Required', 'Part is required.', 'warning');
                    }

                    let vm = this;
                    vm.modalLoading = true;
                    $.get('{{ route('adjust.stock.detail') }}', {
                        part_id: vm.modal.PartID,
                        warehouse_id: vm.warehouse,
                        BatchNo: vm.modal.BatchNo,
                        SerialNo: vm.modal.SerialNo,
                        ExpDate: vm.modal.ExpDate,
                        BIN: vm.modal.BIN,
                        LOC: vm.modal.LOC,
                    }).then(function (result) {
                        let row = Object.assign(result.detail, {
                            InputUnit: vm.modal.InputUnit,
                            InputQty: vm.modal.InputQty,
                        });
                        syncAdjustmentInput(row);
                        row.QtyOpname = calculateAdjustmentPrimaryQty(row);

                        let duplicate = vm.details.some(function (detail, idx) {
                            return idx !== vm.editingIndex && vm.detailKey(detail) === vm.detailKey(row);
                        });
                        if (duplicate) {
                            return Swal.fire('Duplicate Detail', 'The selected stock detail already exists.', 'warning');
                        }

                        if (vm.editingIndex === null) {
                            vm.details.push(row);
                        } else {
                            vm.$set(vm.details, vm.editingIndex, row);
                        }
                        $('#detailModal').modal('hide');
                    }).fail(function (xhr) {
                        Swal.fire('Failed', xhr.responseJSON?.message || 'Unable to fetch stock detail.', 'error');
                    }).always(function () {
                        vm.modalLoading = false;
                    });
                },
                detailKey(detail) {
                    return [detail.PartID, detail.UnitID, detail.BatchNo || '', detail.SerialNo || '', detail.ExpDate || '', detail.BIN || '', detail.LOC || ''].join('|');
                },
                deleteDetail(index) {
                    this.details.splice(index, 1);
                },
                addChecker() {
                    this.checkers.push({ id: makeid(10), employee: '', status: 'SUPERVISOR' });
                },
                deleteChecker(index) {
                    this.checkers.splice(index, 1);
                },
                partText(detail) {
                    if (!detail || !detail.PartID) return '';
                    return detail.PartID + (detail.PartName ? ' - ' + detail.PartName : '');
                },
                displayEmpty(value) {
                ...
                displayQty2(detail) {
                    return formatQty2(calculateAdjustmentQty2(detail), detail.UnitID2);
                },
                displayQty1(detail) {
            ...
            watch: {
                ...
                opname() {
                    if (this.opname && !this.isRestoring) this.getOpname();
                },
                warehouse() {
                    if (this.type === 'manual' && !this.isRestoring) this.details = [];
                }
            },
            mounted() {
                let vm = this;
                vm.isRestoring = true;
                FormPreserver.initVue(vm, 'stock_adjustment_advanced_add', ['isRestoring', 'editingIndex', 'modalLoading', 'modal']);
                vm.$nextTick(function() { vm.isRestoring = false; });

            }
        });

        function thisEmptyModal() {
            return { id: makeid(10), PartID: '', PartName: '', WithSerialNo: 0, UnitID: '', QtyStock: 0, QtyOpname: null, InputUnit: null, InputQty: null, QtyStock2: null, UnitID2: null, BatchNo: null, CoilNo: null, SerialNo: null, ExpDate: null, BIN: null, LOC: null };
        }
        ...
        function initModalSelects(vm) {
            resetModalSelect2('#ModalPart').select2({
                theme: 'bootstrap-5',
            ...
            }).on('select2:select.adjustModal', function (e) {
                let data = e.params.data;
                vm.modal.PartID = data.id;
                vm.modal.PartName = (data.text || '').replace(data.id + ' - ', '');
                vm.modal.WithSerialNo = Number(data.WithSerialNo || 0);
                if (vm.modal.WithSerialNo !== 1) vm.modal.SerialNo = null;
                setSerialSelectState(vm);
                $.get('{{ route('adjust.stock.detail') }}', { part_id: data.id, warehouse_id: vm.warehouse, preview_part: true })
                    .done(function (result) {
                        vm.modal.UnitID = result.detail.UnitID;
                        vm.modal.QtyStock = result.detail.QtyStock;
                        vm.modal.QtyStock2 = result.detail.QtyStock2;
                        vm.modal.UnitID2 = result.detail.UnitID2;
                        vm.modal.QtyOpname = result.detail.QtyStock;
                        vm.modal.InputQty = result.detail.QtyStock;
                        syncAdjustmentInput(vm.modal);
                    });
                $('#ModalBatchNo,#ModalSerialNo,#ModalExpDate,#ModalBIN,#ModalLOC').val(null).trigger('change.select2');
                vm.modal.BatchNo = null;
                vm.modal.CoilNo = null;
                vm.modal.SerialNo = null;
                vm.modal.ExpDate = null;
                vm.modal.BIN = null;
                vm.modal.LOC = null;
            });
        ...
        function selectedStockFilters(vm, excludeTarget) {
            const values = {
                batch_no: vm.modal.BatchNo,
                serial_no: false ? vm.modal.SerialNo : null,
                exp_date: vm.modal.ExpDate,
                bin: vm.modal.BIN,
                loc: vm.modal.LOC,
            };
            const data = {};
            Object.keys(stockDetailColumnMap).forEach(function (key) {
                if (key !== excludeTarget && values[key]) {
                    data[key] = values[key];
                }
            });
            return data;
        ...
        function initCascadingStockSelect(vm, selector, target, field) {
            resetModalSelect2(selector).select2({
                ...
                ajax: {
                    ...
                    data: params => Object.assign({
                        ...
                        target: target,
                        term: params.term || '',
                    }, selectedStockFilters(vm, target)),
            ...
            }).on('change.adjustModal', function () {
                vm.modal[field] = $(this).val() || null;
	                if (field === 'BatchNo') {
	                    const selected = $(this).select2('data')[0] || null;
	                    vm.modal.CoilNo = selected && selected.coil_no ? selected.coil_no : null;
	                    vm.modal.QtyStock = selected && selected.total_qty !== undefined ? Number(selected.total_qty || 0) : vm.modal.QtyStock;
	                    vm.modal.QtyStock2 = selected && selected.total_qty2 !== undefined ? Number(selected.total_qty2 || 0) : vm.modal.QtyStock2;
	                    vm.modal.UnitID2 = selected && selected.unit_id2 ? selected.unit_id2 : vm.modal.UnitID2;
	                    syncAdjustmentInput(vm.modal);
	                }
	            });
	        }
        ...
        $('#TransactionDate').flatpickr({
            dateFormat: "d/m/Y",
            defaultDate: app.transactionDate,
        ...
        $('#ExpiredDate').flatpickr({
            dateFormat: "d/m/Y",
            defaultDate: app.expiredDate,
            onChange: function(selectedDates, dateStr) {
                app.expiredDate = dateStr;
            }
        });
    </script>
@endsection
