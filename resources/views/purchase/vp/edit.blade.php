...
@section('titles')
    <title>Keysoft NLA WMS - Edit Vendor Payment</title>
@endsection
...
@section('content')
    ...
    <div class="px-lg-5 py-lg-3 p-3" id="vue-container">
        <form autocomplete="off" method="post" enctype="multipart/form-data" action="{{ route('vp.update') }}" id="pr-form">
            @csrf
            ...
            <input type="hidden" name="id" value="{{ $vp->TransactionNo }}">
                        ...
                        <div class="col-lg-6 col-12 pe-lg-5">
                            <div class="d-flex flex-row align-items-center mb-3">
                                ...
                                <div class="ms-4">
                                    ...
                                    <input type="text" class="js-flatpickr form-control js-flatpickr-enabled flatpickr-input active" id="TransactionDate" name="TransactionDate" placeholder="d/m/Y" value="{{ date('d/m/Y', strtotime($vp->TransactionDate)) }}" required readonly>
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select2 url="{{ route('misc.currency', ['select2' => true, 'type' => 'CASHANDBANK', 'parent' => true]) }}" class="form-select"  v-model="currency" :prevalue="currency"
                        ...
                        <div class="col-lg-6 col-12 ps-lg-5">
                            ...
                            <div class="row mb-3">
                                <div class="col-7">
                                    ...
                                    <input type="text" class="form-control" id="ChequeNumber" name="ChequeNumber" value="{{ old('ChequeNumber') ?? $vp->ChequeNumber }}">
                                </div>
                                <div class="col-5">
                                    <label class="form-label">Cheque Due Date</label>
                                    <input type="text" class="js-flatpickr form-control js-flatpickr-enabled flatpickr-input active" id="ChequeDueDate" name="ChequeDueDate" placeholder="d/m/Y" value="{{ old('ChequeDueDate') ?? ($vp->ChequeDueDate ? date('d/m/Y', strtotime($vp->ChequeDueDate)) : '') }}" readonly="readonly">
                                </div>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Cheque Bank</label>
                                <input type="text" class="form-control" id="ChequeBank" name="ChequeBank" value="{{ old('ChequeBank') ?? $vp->ChequeBank }}">
                            </div>
                        </div>
                    </div>
                ...
                <div class="block-content">
                    <div class="row justify-content-between mb-3">
                        <div class="col-lg-4 col-12 mb-lg-0 mb-3">
                            ...
                            <textarea name="Notes" class="form-control" rows="3">{{ old('Notes') ?? $vp->Notes }}</textarea>
                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        let app = new Vue({
            ...
            data: {
                isLoading: false,
                url: '{!! route('vp.inv', ['supplier' => $vp->SupplierID, 'currency' => $vp->CurrencyID, 'id' => $vp->id]) !!}',

                supplier: '{{ old('SupplierID') ?? $vp->SupplierID }}',
                currency: '{{ old('CurrencyID') ?? $vp->CurrencyID }}',
                rate: {!! old('Rate') ?? $vp->Rate !!},
                fiscalRate: {!! old('FiscalRate') ?? $vp->FiscalRate !!},
                bankAccount: '{{ old('BankAccountNo') ?? $vp->BankAccountNo }}',
            ...
            watch: {
                supplier(){
                    this.details = [];
                    this.url = '{{ route('vp.inv') }}?supplier=' + this.supplier + '&currency=' + this.currency + "&id=" + '{{ $vp->id }}';
                },
                currency(){
                    setTimeout(function (){
