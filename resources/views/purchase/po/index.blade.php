...
@section('titles')
    <title>Keysoft NLA WMS - Purchase Order</title>
@endsection
            ...
            <div class="flex-grow-1 mb-1 mb-md-0">
                ...
                <div class="row justify-content-between align-items-end mb-3">
                    <div class="col-lg-8 col-12 mb-2">
                        <form autocomplete="off" id="filter-form" class="d-flex flex-wrap align-items-end">
                            <div>
                                <label class="form-label">Period</label>
                                <input type="text"
                                    class="js-flatpickr form-control js-flatpickr-enabled flatpickr-input active"
                                    id="date" name="date" value="{{ request()->get('date' ?? '') }}">
                            </div>
                            <div class="form-check align-self-end ms-3 mb-2">
                                <input class="form-check-input" type="checkbox" value="1" id="outstanding"
                                    name="outstanding" {{ request()->get('outstanding') ? 'checked' : '' }}>
                                <label class="form-check-label" for="outstanding">
                                    Outstanding
                                </label>
                            </div>
                            <button type="submit" class="btn btn-secondary ms-3 mb-2" id="filter">
                    ...
                    <div class="col-lg-auto col-12 mb-2">
                        @if (auth()->user()->hasAnyPermission(['admin', 'po.add']))
                            <a class="btn btn-primary me-2" href="{{ route('po.add') }}">
                                <i class="fa fa-fw fa-plus"></i> Add
                            </a>
                        @endif
                        @if (auth()->user()->hasPermissionTo('admin') ||
                                auth()->user()->hasAllPermissions(['po.add', 'prs.view']))
                            <a class="btn btn-primary" href="{{ route('po.add_from_settle') }}">
                                <i class="fa fa-fw fa-plus"></i> Add From Settle
                            </a>
                        @endif
                    </div>
                </div>
