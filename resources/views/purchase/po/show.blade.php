...
@section('titles')
    <title>Keysoft NLA WMS - Purchase Order - {{ $po->TransactionNo }}</title>
@endsection
...
@section('content')
    ...
    <div class="px-lg-5 py-lg-3 p-3" id="vue-container">
        <div class="d-flex flex-row align-items-center mb-5">
            ...
            <h1 class="h3 fw-bold ms-4 mb-0">{{ $po->TransactionNo }}</h1>
        </div>
        ...
        <div class="row justify-content-between mb-3 align-items-end">
            <div class="col-auto d-flex flex-row">
                @if ($po->Closed == 0)
                    @if (
                        !request()->has('br') &&
                            $po->Editable == 1 &&
                            auth()->user()->hasAnyPermission(['admin', 'po.edit']))
                        <a href="{{ route('po.edit', $po->id) }}" class="btn btn-primary fs-6"><i
                                class="fa fa-fw fa-edit"></i> Edit</a>
                    @endif
                    @if (
                        !request()->has('br') &&
                            auth()->user()->hasAnyPermission(['admin', 'po.close']))
                        <button class="btn btn-secondary ms-2 fs-6" @click="closePO"><i class="fa fa-fw fa-x"></i>
                            Close</button>
                    @endif
                @endif
            </div>
            ...
            <div class="col-auto">
                <form autocomplete="off" action="{{ config('app.report_url') }}/print/index">
                    <input type="hidden" name="transactionCode" value="{{ $po->TransactionNo }}">
                ...
                <div class="row">
                    <div class="col-lg-6 col-12 pe-lg-5">
                        <div class="mb-3">
                            ...
                            <input type="text" name="TransactionNo" id="TransactionNo" class="form-control"
                                value="{{ $po->TransactionNo }}" readonly>
                        </div>
                        <div class="row mb-3">
                            <div class="col-6">
                                <label class="form-label">Transaction Date</label>
                                <input type="text" class="form-control"
                                    value="{{ old('TransactionDate') ?? ($po->TransactionDate != null ? date('d/m/Y', strtotime($po->TransactionDate)) : '') }}"
                                    readonly>
                            </div>
                            <div class="col-6">
                                <label class="form-label">Due Date</label>
                                <input type="text" class="form-control"
                                    value="{{ old('DueDate') ?? ($po->DueDate != null ? date('d/m/Y', strtotime($po->DueDate)) : '') }}"
                                    readonly>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control"
                                value="{{ $po->DivisionID . ($po->division->DivisionName ? ' - ' . $po->division->DivisionName : '') }}"
                                readonly>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control"
                                value="{{ $po->SupplierID . ($po->supplier->SupplierName ? ' - ' . $po->supplier->SupplierName : '') }}"
                                readonly>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-6 col-12 ps-lg-5">
                        <div class="col-6 mb-3" style="width: 100px">
                            ...
                            <input type="text" class="form-control" value="{{ $po->Closed == 1 ? 'Closed' : 'Open' }}"
                                readonly>
                        </div>
                        @if ($po->ClosingReason)
                            <div class="mb-3">
                                <label class="form-label">Closing Reason</label>
                                <input type="text" class="form-control" value="{{ $po->ClosingReason }}" readonly>
                            </div>
                        @endif
                        <div class="mb-3" id="loading-currency">
                            <label class="form-label">Currency</label>
                            <input type="text" class="form-control"
                                value="{{ $po->CurrencyID . ($po->currency->CurrencyName ? ' - ' . $po->currency->CurrencyName : '') }}"
                                readonly>
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-6">
                                ...
                                <input type="text" class="form-control number-input" value="{{ $po->Rate }}"
                                    readonly>
                            </div>
                            <div class="col-6">
                                <label class="form-label">Term Of Payment</label>
                                <input type="text" class="form-control nuber-input" value="{{ $po->Term }}"
                                    readonly>
                            </div>
                        </div>
                    ...
                    <table class="table table-bordered nowrap w-100" style="table-layout: fixed; overflow-x: scroll">
                        ...
                        <tbody>
                            <tr v-for="(detail, index) in details">
                                @if ($po->fromSettle == 1)
                                    <td>
                                        <input type="text" v-model="detail.settleNo" class="form-control" readonly>
                                    </td>
                                @endif
                                <td>
        ...
        @include('purchase.po.close_modal')
    </div>
    <!-- END Hero -->
    ...
    <script>
        let app = new Vue({
            ...
            data: {
                ...
                selectedImg: '',
                subTotal: 0,
                serviceTax: '{{ $po->ServiceTax }}',
                pph: '{{ $po->PercentagePPH }}',
                {{-- //pph22: '{{ old('PPH22') ?? $po->PPH22 }}', --}}
                pph22: 0,
                {{-- pph22Percentage: '{{ $po->PPH22 / $po->SubTotal * 100 }}', --}}
                pph22Percentage: 0,
                freight: {{ $po->Freight }},
                vatType: '{{ $po->VAT }}',
