...
@section('titles')
    <title>Keysoft NLA WMS - Edit Purchase Order</title>
@endsection
...
@section('content')
    ...
    <div class=" w-100 px-lg-5 py-lg-3 p-3" id="vue-container" style="overflow-x: hidden">
        ...
        <form autocomplete="off" method="post" enctype="multipart/form-data" action="{{ route('po.update') }}" id="po-form">
            @csrf
            ...
            <input type="hidden" name="id" value="{{ $po->TransactionNo }}">
                        ...
                        <div class="col-lg-6 col-12 pe-lg-5">
                            <div class="row mb-3">
                                ...
                                <div class="col-6">
                                    ...
                                    <input type="text"
                                        class="js-flatpickr form-control js-flatpickr-enabled flatpickr-input active"
                                        id="DueDate" name="DueDate" placeholder="d/m/Y"
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-6">
                                    ...
                                    <input type="text" class="form-control number-input" name="Term"
                                        id="Term" value="{{ old('Term') ?? $po->Term }}">
                                </div>
                            </div>
                ...
                <div class="block-content">
                    <div class="d-flex flex-row justify-content-between align-items-center mb-3">
                        ...
                        <div class="d-flex flex-row">
                            @if ($po->fromSettle == 0)
                                <button type="button" class="btn btn-primary" @click="addDetail"><i
                                        class="fa fa-fw fa-plus me-1"></i>Add Detail</button>
                            @endif
                        ...
                        <table class="table table-bordered nowrap w-100" style="table-layout: fixed; overflow-x: scroll">
                            ...
                            <tbody>
                                ...
                                <tr v-for="(detail, index) in details" :key="detail.id">
                                    <td hidden>
                                        <input type="text" name="settleDtID[]" class="form-control"
                                            v-model="detail.settleDtID" readonly>
                                    </td>
                                    <td hidden>
                                        <input type="text" name="settleNo[]" class="form-control"
                                            v-model="detail.settleNo" readonly>
                                    </td>
                                    <td>
    ...
    <script>
        ...
        let app = new Vue({
            ...
            data: {
                ...
                selectedImg: '',
                subTotal: 0,
                serviceTax: '{{ old('ServiceTax') ?? $po->ServiceTax }}',
                pph: '{{ old('PercentagePPH') ?? $po->PercentagePPH }}',
                {{-- pph22: '{{ old('PPH22') ?? $po->PPH22 }}', --}}
                pph22: 0,
                pph22Percentage: {{ old('PercentagePPH22') ?? '0' }},
                freight: '{{ old('Freight') ?? $po->Freight }}',
                vatType: '{{ old('VAT') ?? $po->VAT }}',

                init: false,
                currency: '{{ old('CurrencyID') ?? $po->CurrencyID }}',
                editType: '',
        ...
        $.ajax({
            ...
            if (data.status == 'success') {
                var options = '';
                var oldID = '{{ old('DivisionID') ?? $po->DivisionID }}';
        ...
        $.ajax({
            ...
            if (data.status == 'success') {
                var options = '';
                var oldID = '{{ old('SupplierID') ?? $po->SupplierID }}';
                for (var i = 0; i < data.data.length; i++) {
                    options +=
                        `<option value="${data.data[i].id}" ${ oldID == data.data[i].id ? 'selected' : '' }>${data.data[i].id}${data.data[i].text != null ? ' - ' + data.data[i].text : ''}</option>`;
                }
        ...
        {{--    if (data.status == 'success'){ --}}
        {{--        var options = ''; --}}
        {{--        var oldID = '{{ old("CurrencyID") ?? $po->CurrencyID }}'; --}}
        {{--        for(var i = 0; i < data.data.length; i++){ --}}
