...
@section('titles')
    <title>Keysoft NLA WMS - Edit Direct Purchase</title>
@endsection
...
@section('content')
    ...
    <div class=" w-100 px-lg-5 py-lg-3 p-3" id="vue-container" style="overflow-x: hidden">
        ...
        <form autocomplete="off" method="post" enctype="multipart/form-data" action="{{ route('dp.update') }}" id="po-form">
            @csrf
            ...
            <input type="hidden" name="id" value="{{ $dp->TransactionNo }}">
                        ...
                        <div class="col-lg-6 col-12 pe-lg-5">
                            <div class="row mb-3">
                                <div class="col-6">
                                    ...
                                    <input type="text" class="js-flatpickr form-control js-flatpickr-enabled flatpickr-input active" id="TransactionDate" name="TransactionDate" placeholder="d/m/Y" value="{{ old('TransactionDate') ?? ($dp->TransactionDate != null ? date('d/m/Y', strtotime($dp->TransactionDate)) : '') }}" required readonly>
                                </div>
                                <div class="col-6">
                                    <label class="form-label">Due Date <span class="text-danger">*</span></label>
                                    <input type="text" class="js-flatpickr form-control js-flatpickr-enabled flatpickr-input active" id="DueDate" name="DueDate" placeholder="d/m/Y" value="{{ old('DueDate') ?? ($dp->DueDate != null ? date('d/m/Y', strtotime($dp->DueDate)) : '') }}" readonly="readonly" required>
                                </div>
                            </div>
...
{{--                            </div>--}}
                            <div class="mb-3" id="currency-container">
                                <label class="form-label">Currency <span class="text-danger">*</span></label>
                                <select2 url="{{route('misc.currency', ['select2' => true])}}" v-model="currency" :prevalue="currency" class="form-select" name="CurrencyID" placeholder="Pick Currency" required>
                                    <option>-</option>
                                </select2>
                            </div>
                            ...
                            <div class="row mb-3">
                                <div class="col-6">
                                    ...
                                    <input type="text" class="form-control" name="Rate" id="Rate" value="{{ old('Rate') ?? $dp->Rate }}">
                                </div>
                                <div class="col-6">
                                    <label class="form-label">Fiscal Rate <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control number-input" name="FiscalRate" id="FiscalRate" value="{{ old('FiscalRate') ?? $dp->FiscalRate }}">
                                </div>
                            </div>
                ...
                <div class="block-content">
                    <div class="d-flex flex-row justify-content-between align-items-center mb-3">
                        ...
                        <div class="d-flex flex-row">
                            ...
                            <button type="button" class="btn btn-secondary ms-1" @click="addRev"><i class="fa fa-fw fa-plus me-1"></i>Add Rev</button>
                            <button type="button" class="btn btn-secondary ms-1" @click="removeRev"><i class="fa fa-fw fa-minus me-1"></i>Decrease Rev</button>
                        </div>
                    </div>
                    ...
                    <div id="detail-table" class="table-responsive w-100" style="overflow-x: scroll">
                        <table class="table table-bordered nowrap w-100" style="table-layout: fixed; overflow-x: scroll">
                            ...
                            <tr v-for="(detail, index) in details" :key="detail.id">
                                ...
                                <td>
                                    <select2 :url="detail.unitUrl" v-model="detail.unit" :prevalue="detail.unit" class="form-control unit" name="unit[]" :id="'unit'+index" :disabled="detail.part == null" placeholder="Pick Unit" :data-index="index" required>
                                    </select2>
                                </td>
                                <td>
                                ...
                                <td>
                                    <vue-autonumeric :options="autonumericFormat2" name="price[]" v-model="detail.price" class="form-control" required></vue-autonumeric>
                                </td>
                                <td>
                                    <vue-autonumeric :options="autonumericFormat2" name="discount1p[]" v-model="detail.discount1p" class="form-control"></vue-autonumeric>
                                </td>
                                <td>
                                    <vue-autonumeric :options="autonumericFormat2" name="discount1[]" v-model="detail.discount1" class="form-control"></vue-autonumeric>
                                </td>
                                <td>
                                    <vue-autonumeric :options="autonumericFormat2" name="discount2p[]" v-model="detail.discount2p" class="form-control"></vue-autonumeric>
                                </td>
                                <td>
                                    <vue-autonumeric :options="autonumericFormat2" name="discount2[]" v-model="detail.discount2" class="form-control"></vue-autonumeric>
                                </td>
                                <td>
                                    <input type="hidden" name="discount[]" v-model="detail.totalDiscount">
                                    <vue-autonumeric :options="autonumericFormat2" name="total_price[]" class="form-control" :value="totalPrice(index)" readonly></vue-autonumeric>
                                </td>
                                <td>
                                    <select2 url="{{route('misc.division2')}}" v-model="detail.division" :prevalue="detail.division" class="form-select" name="division[]" placeholder="Pick Division" required>
                                        <option>-</option>
                                    </select2>
                                </td>
                                <td>
                                    <select2 url="{{route('misc.warehouse2')}}" v-model="detail.warehouse" :prevalue="detail.warehouse" class="form-select" name="warehouse[]" placeholder="Pick Warehouse" :data-index="index" required>
                                        <option>-</option>
                                    </select2>
                                </td>
                                        ...
                                        <tr>
                                            ...
                                            <td class="px-2">
                                                <div class="d-flex flex-row align-items-center">
                                                    <vue-autonumeric :options="autonumericFormat2" name="PercentageServiceTax" v-model="pph" data-type="pph" class="form-control sync-edit" style="width: 50px"></vue-autonumeric>
                                                    <p class="mb-0 ms-1 fs-6">%</p>
                                                </div>
                                            </td>
                                            <td>
                                                <vue-autonumeric :options="getFormat()" class="form-control text-end sync-edit" name="ServiceTax" v-model="serviceTax" data-type="serviceTax"></vue-autonumeric>
                                            </td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <vue-autonumeric :options="autonumericFormat" class="form-control text-end" :value="getVAT" readonly></vue-autonumeric>
                                            </td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td></td>
                                            <td>
                                                <vue-autonumeric :options="autonumericFormat" name="Freight" class="form-control text-end" v-model="freight"></vue-autonumeric>
                                            </td>
                                        </tr>
                                        <tr>
    ...
    <script>
        ...
        let app = new Vue({
            ...
            data: {
                ...
                selectedImg: '',
                subTotal: 0,
                serviceTax: '{{ old('ServiceTax') ?? $dp->ServiceTax }}',
                pph: '{{ old('PercentageServiceTax') ??  $dp->PercentageServiceTax }}',
                freight: '{{ old('Freight') ?? $dp->Freight }}',
                vatType: '{{ old('VAT') ?? $dp->VAT }}',
            ...
            mounted() {
                let app = this;
                $('#po-form').on('change', '.part', function (){
                    if($(this).val() != null){
        ...
        $.ajax({
            ...
            if (data.status == 'success'){
                var options = '';
                var oldID = '{{ old("DivisionID") ?? $dp->DivisionID }}';
        ...
        $.ajax({
            ...
            if (data.status == 'success'){
                var options = '';
                var oldID = '{{ old("SupplierID") ?? $dp->SupplierID }}';
                for(var i = 0; i < data.data.length; i++){
                    options += `<option value="${data.data[i].id}" ${ oldID == data.data[i].id ? 'selected' : '' }>${data.data[i].id}${data.data[i].text != null ? ' - ' + data.data[i].text : ''}</option>`;
                }
        ...
        {{--    if (data.status == 'success'){--}}
        {{--        var options = '';--}}
        {{--        var oldID = '{{ old("CurrencyID") ?? $dp->CurrencyID }}';--}}
        {{--        for(var i = 0; i < data.data.length; i++){--}}
        {{--            options += `<option value="${data.data[i].id}" ${ oldID == data.data[i].id ? 'selected' : '' }>${data.data[i].id}${data.data[i].text != null ? ' - ' + data.data[i].text : ''}</option>`;--}}
        {{--        }--}}
