...
@section('titles')
    <title>Keysoft NLA WMS - Edit Division</title>
@endsection
    ...
    <div class="px-lg-5 py-lg-3 p-3">
        <div class="d-flex flex-column flex-md-row justify-content-md-between align-items-md-center py-2 text-md-start">
            <div class="flex-grow-1 mb-1 mb-md-0">
                ...
                <form autocomplete="off" method="post" enctype="multipart/form-data" action="{{ route('user.division.update') }}">
                    @csrf
                            ...
                            <div class="row">
                                <div class="col-lg-3 col-12">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="DivisionID" class="form-control" value="{{ old('DivisionID') ?? $division->DivisionID }}" readonly>
                                    </div>
                                </div>
                                <div class="col-3 px-lg-0">
                                    <label class="form-label"></label>
                                    <div class="form-check ms-lg-4 pt-lg-2">
                                        <input class="form-check-input fs-6" type="checkbox" value="1" name="Active" id="Active" {{ old('Active') ? 'checked' : ($division->Active == 1 ? 'checked' : '') }}>
                            ...
                            <div class="row">
                                <div class="col-lg-5 col-12">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="DivisionName" value="{{ old('DivisionName') ?? $division->DivisionName }}"class="form-control">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-lg-8 col-12">
                                    <div class="mb-3">
                                        ...
                                        <textarea name="Notes" class="form-control" rows="3">{{ old('Notes') ?? $division->Notes }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        $(function() {
            ...
            $.ajax({
                ...
                if (data.status == 'success'){
                    var options = '';
                    var oldID = '{{ old("SubDivisionID") ?? $division->SubDivisionID }}';
            ...
            $.ajax({
                ...
                if (data.status == 'success'){
                    var options = '';
                    var oldID = '{{ old("StaffInChargeID") ?? $division->StaffInChargeID }}';
                    for(var i = 0; i < data.data.length; i++){
                        options += `<option value="${data.data[i].id}" ${ oldID == data.data[i].id ? 'selected' : '' }>${data.data[i].id}${data.data[i].text != null ? ' - ' + data.data[i].text : ''}</option>`;
                    }
