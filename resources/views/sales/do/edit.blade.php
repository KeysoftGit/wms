...
@section('titles')
    <title>Keyonline - Edit Advanced Delivery Order</title>
@endsection

@section('content')
    <div class="px-lg-5 py-lg-3 p-3" id="vue-container">
        <form autocomplete="off" method="post" enctype="multipart/form-data" action="{{ route('do.update') }}" id="do-form">
            @csrf
            <input type="hidden" name="id" value="{{ $do->TransactionNo }}">
            ...
            <div class="d-flex flex-row align-items-center mb-5">
                ...
                <h1 class="h3 fw-bold ms-4 mb-0">Edit Delivery Order</h1>
            </div>
                ...
                <div class="block-content pb-3">
                    ...
                    <div class="row align-items-center mb-3">
                        <div class="col-lg-2 col-12 pe-lg-0">
                            ...
                            <input type="text" id="TransactionNo" class="form-control" value="{{ $do->TransactionNo }}" readonly>
                        </div>
                    </div>
                        ...
                        <div class="col-lg-6 col-12 pe-lg-5">
                            <div class="row mb-3">
                                <div class="col-lg-4 col-12 mb-lg-0 mb-3">
                                    ...
                                    <input type="text" class="js-flatpickr form-control" id="TransactionDate" name="TransactionDate" placeholder="d/m/Y" value="{{ old('TransactionDate') ?? ($do->TransactionDate != null ? date('d/m/Y', strtotime($do->TransactionDate)) : '') }}" readonly required>
                                </div>
                                <div class="col-lg-4 col-12 mb-lg-0 mb-3">
                                    <label class="form-label">Expired <span class="text-danger">*</span></label>
                                    <input type="text" class="js-flatpickr form-control" id="ExpiredDate" name="ExpiredDate" placeholder="d/m/Y" value="{{ old('ExpiredDate') ?? ($do->ExpiredDate != null ? date('d/m/Y', strtotime($do->ExpiredDate)) : '') }}" readonly required>
                                </div>
                                <div class="col-lg-4 col-12">
                                    <label class="form-label">ETA <span class="text-danger">*</span></label>
                                    <input type="text" class="js-flatpickr form-control" id="ETA" name="ETA" placeholder="d/m/Y" value="{{ old('ETA') ?? ($do->ETA != null ? date('d/m/Y', strtotime($do->ETA)) : '') }}" readonly required>
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select2 :url="soURL" v-model="ReffNumber" :prevalue="ReffNumber" @selected_text="ReffNumberText = $event" class="form-select" id="ReffNumber" name="ReffNumber" placeholder="Pick Sales Order" :disabled="!CustomerID" required>
                                    <option>-</option>
                                    @if(!old('ReffNumber'))
                                        <option value="{{ $reffId }}" selected>{{ $do->ReffNumber }}</option>
                                    @endif
                                </select2>
                            </div>
                        </div>
    ...
    <script>
        ...
        let app = new Vue({
            ...
            data: {
                ...
                CustomerID: '{{ old('CustomerID') ?? $customerId }}',
                CustomerText: @json($customer),
                ReffNumber: '{{ old('ReffNumber') ?? $reffId }}',
                ReffNumberText: @json($do->ReffNumber),
                prevSO: '{{ $reffId }}',
                doID: '{{ $do->TransactionNo }}',
                Shipment: '{{ old('Shipment') ?? '' }}',
                ShipmentAddress: @json(old('ShipmentAddress') ?? $do->ShipmentAddress),
                Notes: @json(old('Notes') ?? $do->Notes),
                addressUrl: '',
                soURL: '',
                modalSoDetails: [],
                ...
                getDetail() {
                    ...
                    $.ajax({
                        ...
                        if (result.status === 'success') {
                            ...
                            if (this.details.length === 0 && result.selected_data && result.selected_data.length > 0) {
                                this.details = result.selected_data;
                            }
                        }
                    }).always(() => {
            ...
            mounted() {
                this.addressUrl = '{{ route('misc.shipment') }}?id=' + this.CustomerID + '&numId=true';
                this.soURL = '{!! route('do.so') !!}?customer=' + this.CustomerID + '&inv=' + this.ReffNumber;
                this.$nextTick(() => {
                    setTimeout(() => {
                        ...
                        if (this.ReffNumber && this.modalSoDetails.length === 0) {
                            this.getDetail();
                        }
                    }, 100);
                });
