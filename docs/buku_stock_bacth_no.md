...
- Transaction controllers yang insert atau consume `Buku_Stock`
- API transaction controllers yang insert atau consume `Buku_Stock`
...
Semua caller helper harus mengirim maksimal tiga argumen tersebut. Jangan
mengirim `SerialNo`, `ExpDate`, `BIN`, atau `LOC` ke helper stok.
...
Audit semua caller dan ubah pemanggilan helper supaya tidak mengirim field
identitas lama.

### Phase 4: Modul Transaksi API

Update modul API yang create, edit, delete, atau validate row `Buku_Stock`.
Untuk phase ini, fokus hanya API yang dipakai Flutter. Web/Blade dikerjakan
nanti di phase terpisah.
